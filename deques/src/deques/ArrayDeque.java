...
public class ArrayDeque<T> extends AbstractDeque<T> {
    ...
    private static int increment(int i, int length) {
        if (i == length - 1) { // if it's on the last node of array, it goes to the first in the array.
            return 0;
        } else {
            return i + 1;
        }
    ...
    private void resize(int capacity) {
        ...
        // when decreasing the array, the size is not affect the length of the existing array length.
        int i = increment(front, data.length);
